...
def get_dsm_loss(model, x, sigma: torch.Tensor, conditioning=None, ic=None, pde_id=None):
    ...
    pde_id: (B,) long or None — passed through to multi-PDE backbones;
            single-PDE backbones ignore it.
    """
    ...
    # Denoised prediction; pde_id is forwarded for multi-PDE backbones.
...
def get_bv_loss(model, x, sigma: torch.Tensor, conditioning=None, ic=None, pde_id=None):
    r"""
    Total-Variation (TV) penalty (see paper for the L_BV definition).
    conditioning: 条件张量 — 拼接到带噪输入 (推荐新接口)
    ic: 向后兼容旧参数名
    pde_id: passed through to multi-PDE backbones
    """
    cond = conditioning if conditioning is not None else ic
    noise = torch.randn_like(x)
    sigma = sigma.view(-1, 1, 1).to(x.device)
...
def get_godunov_time_loss(model, x_prev, x_target, sigma, dt, dx, conditioning=None, ic=None,
    ...
    ic: 向后兼容旧参数名
    pde_id: (B,) long or None — currently not used for per-sample flux dispatch;
            the flux is chosen by the flux_type string (one PDE per batch).
    ...
    # Dispatch the Godunov flux by name (defaults to Burgers).
...
def godunov_flux_bl(ul, ur):
    """
    Godunov flux for the non-convex Buckley–Leverett equation.
    f(u) = u^2 / (u^2 + (1-u)^2),  u ∈ [0, 1].

    Wraps `src/pdes/bl_flux.py::bl_godunov_flux` to share the (ul, ur) → f interface.
    """
    # Lazy import to avoid a circular dependency between `src.pdes` and `src.diffusion`.
...
def godunov_flux_euler_density(ul, ur):
    """
    Approximate Godunov flux for the Euler density channel.

    严格 Euler 密度方程: ρ_t + (ρu)_x = 0
    单独看 ρ 通道时缺少 u, 无法严格守恒律. 用 Burgers Godunov 作代理:
    ρ 当作"速度"处理. 这仅作为 time loss 的物理一致性正则项, 不影响主 W₁.

    This is an approximate flux: the density channel is treated as an advected scalar.
    """
    return godunov_flux(ul, ur)


# ---- Flux registry: dispatch from a string name to the corresponding function ----
# Add a new PDE flux by extending this registry; no change to `get_godunov_time_loss` is needed.
...
def pde_residual(u, dx: float, flux_type: str = "burgers"):
    ...
    flux_type: name of the Godunov flux to use; defaults to Burgers for backwards compatibility.
    """
    flux_fn = _resolve_flux_fn(flux_type)
    ul = u[:, :, :-1]
    ur = u[:, :, 1:]
